...
def _render_selection(cfg: dict, result: RunResult, qc_report: QCReport) -> str:
    ...
    return (
        ...
        f"{lead}"
        f"Taxonomic diversity at each rank across three pipeline stages "
        f"(pre-QC, pre-clustering, and representatives — so a whole genus or "
        f"family eliminated by a QC filter is visible rather than silent), "
        f"with per-taxon {rep_unit.split()[-1]} counts (the top 20 by member "
        f"count for high-diversity ranks), is reported in "
        f"`{{prefix}}_taxonomic_report.txt`; any genus-or-higher taxon present "
        f"in the input but with zero post-QC survivors is additionally flagged "
        f"in `{{prefix}}_flags.txt` and on the console. When polyprotein "
        f"slicing is configured, a clade whose peptides are almost entirely "
        f"uncovered by its best-matching spec (a peptide-coverage \"wall of "
        f"zeros\", i.e. mistuned or missing profiles for that clade) is flagged "
        f"the same way. A parallel "
        ...
        f"When the representatives span more than one viral subtype "
        f"(serotype, e.g. influenza H5N1), a subtype-distribution report "
        f"(`{{prefix}}_subtype_report.txt` and a tidy "
        f"`{{prefix}}_subtype_report.tsv` sharing the same 8-column schema) "
        f"is also written, giving each serotype's pre- and post-clustering "
        f"{rep_unit.split()[-1]} count. "
...
def _render_phylo(cfg: dict, result: RunResult, phylo_ran: bool,
    ...
    if pre_cluster_ran:
        paragraphs.append(
            ...
            f"v{versions.get('mafft') or '?'} ({_CITATIONS['mafft']}) "
            "with `--retree 1` (FFT-NS-1, no `--auto`; very large pools "
            "switch to `--parttree` to avoid running out of memory), "
            "followed by "
            f"**FastTree** v{versions.get('FastTree') or '?'} "
    ...
    if msa_ran:
        paragraphs.append(
            ...
            "incongruence table. That cross-tree comparison is pivoted into "
            "`{prefix}_segment_status_matrix.tsv`, whose `single_marker_break` "
            "column names the one marker tree each taxon breaks on while staying "
            "clean on the whole-genome tree — the localised reassortment "
            "candidate list. The para/poly split is a topology-only "
            "heuristic (keyed on `intruder_clusters`); the supporting counts "
            "(`n_clusters`, `n_intruders`, `intruder_clusters`) are reported "
            "alongside so the call can be checked by hand. The assessment is "
